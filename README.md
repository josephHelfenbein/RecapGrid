...
8. Enable Cloud Tasks API on Google Cloud Platform, create a queue named recapgrid-video-queue, set its max concurrent dispatches to 1, and add the project ID to `GCP_PROJECT_ID`, the queue region to `GCP_LOCATION`, the service account email to `GCP_SERVICE_ACCOUNT`, and the backend URL to `GCP_BASE_URL` environment variables.
